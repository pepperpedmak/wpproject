...
import { fetchTeamData, addTeam, getCookie } from "../serverAction/serverAction";
...
const TeamSideBar: React.FC = () => {
  ...
  useEffect(() => {
    // Wrap the async operation in a function inside useEffect
  ...
  const loadTeamData = async (userId?: string) => {
    ...
    try {
      ...
      // Identify pending teams
      const pendingTeamIds = teams
        .filter((team) => {
          console.log(`Checking team ${team.teamName}:`, team.users);
          return team.users?.some(
            (user) =>
              user.user._id === userId && user.status === "pending"
          );
        })
        .map((team) => team._id);

      setPendingTeams(new Set(pendingTeamIds));
    } catch (error) {
  ...
  const handleSubmitTeam = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await loadTeamData(currentUserId || undefined);
    } catch (error) {
    ...
    <div className="bg-white p-4 w-64 shadow-xl">
      ...
      {teamData.map((team) => (
        <TeamCard
          ...
          isPending={pendingTeams.has(team._id)}
          refreshTeamData={() => loadTeamData(currentUserId || undefined)}
        />
      ))}
    </div>
  );
